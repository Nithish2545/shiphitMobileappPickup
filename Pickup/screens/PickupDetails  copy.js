...
const PickupDetails = () => {
  ...
  const handleRemoveFile = (fileName, setState) => {
    setState(prevFiles => prevFiles.filter(file => file.fileName !== fileName));
  };
      ...
      {details ? (
        <View>
          ...
          <View style={styles.detailContainer}>
            ...
            <Text style={styles.text}>{details.pickupInstructions || '-'}</Text>
          </View>
          ...
          <View style={styles.formContainer}>
            ...
            <View style={styles.quantityContainer}>
              <Button
                title="-"
                onPress={() => setNumberOfPackages(Math.max(1, numberOfPackages - 1))}
                color="#8447D6"
              />
              <TextInput
                style={styles.quantityInput}
                value={numberOfPackages.toString()}
                onChangeText={(text) => setNumberOfPackages(Math.max(1, parseInt(text, 10)))}
                keyboardType="numeric"
              />
              <Button
                title="+"
            ...
            <FileInput
              ...
              files={productImages}
              onAddFiles={() => handleFileChange('PRODUCT IMAGES', setProductImages)}
              onRemoveFile={(fileName) => handleRemoveFile(fileName, setProductImages)}
            />
            <FileInput
              label="Package Weight Images (1-5)"
              files={packageWeightImages}
              onAddFiles={() => handleFileChange('PACKAGE WEIGHT', setPackageWeightImages)}
              onRemoveFile={(fileName) => handleRemoveFile(fileName, setPackageWeightImages)}
            />
            <FileInput
              label="Form Images (1-2)"
              files={formImages}
              onAddFiles={() => handleFileChange('FORM IMAGES', setFormImages)}
              onRemoveFile={(fileName) => handleRemoveFile(fileName, setFormImages)}
            />
            ...
            />
            {submitLoading && <ActivityIndicator size="large" color="#8447D6" />}
          </View>
        </View>
      ) : (
        <Text style={styles.errorText}>No details found for this AWB number.</Text>
      )}
    </ScrollView>
  );
};
